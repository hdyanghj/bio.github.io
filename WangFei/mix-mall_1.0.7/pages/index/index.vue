	...
	<view class="container">
		...
		<view class="carousel-section">
			...
			<swiper class="carousel" circular @change="swiperChange">
				<swiper-item v-for="(item, index) in homeList.advertiseList" :key="index" class="carousel-item" @click="navToDetailPage(item.id)">
					<image :src="item.pic" />
				</swiper-item>
			</swiper>
			<!-- 自定义swiper指示器 -->
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			async loadData() {
				let homeList = await request.getData('','home/content');
				console.log(homeList)
				let carouselList = await this.$api.json('carouselList');
				this.titleNViewBackground = homeList.advertiseList[0].pic;
				this.swiperLength = homeList.advertiseList.length;
			...
			swiperChange(e) {
				...
				this.titleNViewBackground = this.homeList.advertiseList[0].pic;
			},
			//详情页
			navToDetailPage(id) {
				//测试数据没有写id，用title代替
